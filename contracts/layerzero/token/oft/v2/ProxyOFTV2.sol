...
contract ProxyOFTV2 is BaseOFTV2 {
    ...
    constructor(
        ...
        address _lzEndpoint
    ) BaseOFTV2(_sharedDecimals, _lzEndpoint) Ownable(_msgSender()) {
        innerToken = IERC20(_token);

        (bool success, bytes memory data) = _token.staticcall(abi.encodeWithSignature("decimals()"));
        require(success, "FTD"); // ProxyOFT: failed to get token decimals
        uint8 decimals = abi.decode(data, (uint8));

        require(_sharedDecimals <= decimals, "SDH"); // sharedDecimals must be <= decimals
    ...
    function _debitFrom(
        ...
        require(_from == _msgSender(), "ONC"); // ProxyOFT: owner is not send caller
        ...
        require(cap >= outboundAmount, "OAO"); // ProxyOFT: outboundAmount overflow
